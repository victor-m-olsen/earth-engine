...
var validation_v1 = ee.FeatureCollection('users/dmdruce/victor/validation-sample')
var distMask2 = validation_v1.distance(1000).mask().not()


var validation_v2 = i.updateMask(distMask).updateMask(distMask2)
              .stratifiedSample({
  numPoints:1, 
  classBand:'remapped', 
  region: geometry, 
  ...
  classValues: [5],  ///buffer, savannah, Forest, Bare, Crop
  classPoints: [100], 
  dropNulls: true, 
  geometries: true
})
...
print(validation_v2)
Map.addLayer(validation_v2, {}, 'new_sample')
Export.table.toDrive(validation_v2, 'validation_v2')


Map.addLayer(validation_v1, {}, 'validation_v1')
