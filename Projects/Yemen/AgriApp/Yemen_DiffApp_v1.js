...
var imageVisParams = {"bands":["red","green","blue"],"gamma":1,"max":2875.28,"min":200.72,"opacity":1},
    ...
    WaPor_LULC17 = ee.Image("users/pedrovieirac/WaPor_LULC_17"),
    WaPor_LULC18 = ee.Image("users/pedrovieirac/WaPor_LULC_18"),
    WaPor_LULC19 = ee.Image("users/pedrovieirac/WaPor_LULC_19"),
    WaPor_LULC20 = ee.Image("users/pedrovieirac/WaPor_LULC_20"),
    table = ee.FeatureCollection("users/pedrovieirac/Yemen/yem_adm3_polygons"),
...
var imageNameBase_diff = "users/pedrovieirac/Yemen/Diff_NDVI_"
...
var imageNameList_diff =  ee.List([]);
var i,j;
for (j= 2020; j < 2021; j++){
for (i = 1; i < 13; i++) {
  if(j<2021 ||(j == 2021 && i < 8)){
  var file_name = imageNameBase_diff.concat(j).concat("_").concat(i);
  var image = ee.Image(file_name)
  var image = image.set("month", i)
  var image = image.set("year", j)
  imageNameList_diff = imageNameList_diff.add(image)}
}}
...
print(imageNameList_diff, "Image Difference List")

print(imageNameList, "Image List")

// Map.addLayer(ee.Image(imageNameList[0]),imageVisParamsNDVI, 'NDVI Asset 1', false)


// Creating the slider collections and setting DATE property for collections
var slider_collect_diff = ee.ImageCollection(imageNameList_diff)
var slider_collect = ee.ImageCollection(imageNameList)


     slider_collect_diff = slider_collect_diff.map(function(image){
       var m = image.get("month")
       var y= image.get("year")
       return image.toFloat().set('system:time_start', ee.Date.fromYMD(y, m, 1)).set("month",m).set("year",y);
     })
...
// Map.addLayer(slider_collect.first(),imageVisParamsNDVI, 'NDVI Asset 1', false)





var chart_collect_diff = slider_collect_diff
var chart_collect = slider_collect

 

// var filtered_collect = chart_collect.map(function(image){    // IMPORTANT TO CHANGE THIS BACK 
...
var chart_series = function(geo) {return ui.Chart.image.series(chart_collect, geo, ee.Reducer.mean(), 100)
      .setOptions({
...
var adm1_names = yem_adm3.aggregate_array("ADM1_EN").distinct().sort().getInfo()
var adm2_names = yem_adm3.aggregate_array("ADM2_EN").distinct().sort().getInfo()
var adm3_names = yem_adm3.aggregate_array("ADM3_EN").distinct().sort().getInfo()
...
var update_widgets = function(feature, level) {
  if(level ==1){
  var adm2_filtered = feature.aggregate_array("ADM2_EN").distinct().sort().getInfo()
  var adm3_filtered = feature.aggregate_array("ADM3_EN").distinct().sort().getInfo()
...
var zoomToAdm1 = function(key) {
    var feature = yem_adm3.filter(ee.Filter.eq('ADM1_EN', key))
...
var zoomToAdm2 = function(key) {
  var feature = yem_adm3.filter(ee.Filter.eq('ADM2_EN', key))
...
var zoomToAdm3 = function(key) {
  var feature = yem_adm3.filter(ee.Filter.eq('ADM3_EN', key))
    print(key,"adm3")
    print(feature)
...
var showMosaic = function(range) {
  ...
  var dateSlider = ui.DateSlider({      // INSERTION OF VALUES MANUALLY SHOULD BE UPDATED
    start: 1577836800000,
    end: 1609286400000,
    value: null,
    period: 30,
    onChange: showMosaic,
     style: {width: '35%',backgroundColor: 'ffffff50', 
...
var palette =['660066', '66006650', '00660000', '00660050', '006600'];
var names = ['NDVI: -1','NDVI: -0.5','NDVI: ≈0 ','NDVI: 0.5','NDVI: 1'];
...
//Application Tittle
var title = ui.Label({
  value: 'NDVI of Agricultural Areas in Yemen 2020-2021',
  style: {
    position: 'top-center',
    backgroundColor: 'ffffff22',
      // fontWeight: 'light',
