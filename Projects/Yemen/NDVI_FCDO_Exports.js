...
/////////////////////////////// APPLYING CROP MASKS AND ANALYZING PIXELS WITHIN 

// var crop_mask = GLC10_LULC.eq(10)
var crop_mask = WaPor_LULC.eq(42)

var clipped_mosaics = sentinel2_mosaics.map(function(image){    
  return image.updateMask(crop_mask)})
  
var filtered_mosaic = clipped_mosaics.first().updateMask(clipped_mosaics.first().select("NDVI").neq(0))

var filtered_mosaics = clipped_mosaics.map(function(image){    
  return image.updateMask(image.select("NDVI").neq(0))})
...
Map.addLayer(filtered_mosaics.filterMetadata('month', 'equals', 4), imageVisParamNDVI, 'Filtered 4', false)
...
/////////////////////////////// ZONAL STATISTICS FOR YEMEN ADM AREAS

var yem_adm1_test = yem_adm3.filter(ee.Filter.eq('ADM1_PCODE', 'YE18'));
...
// Output: id, mean and date extracted for each image, for each feature
  var triplets = imageCol.map(function(image) {
    return image.reduceRegions({
                    collection: yem_adm1_test,          //UPDATE FEATURE COLLECTIONS HERE
                    reducer: ee.Reducer.mean(), 
                    scale: 10,
                    tileScale: 16
                 })
...
format_function = format_function(filtered_mosaics, 'date', 'ADM3_PCODE')

print(format_function)

// print(ee.Feature(format_function.first()).select(filtered_mosaics.first().bandNames()));

print(ee.Feature(format_function.first()));



/// EXPORT ///
var description = 'YE18_Test3'
