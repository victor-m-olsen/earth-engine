...
var geom_display = 
    ...
    ee.Geometry.Polygon(
        [[[37.958641516606065, 36.836889102503186],
          [37.958641516606065, 36.626396185657384],
          [38.21785019580528, 36.626396185657384],
          [38.21785019580528, 36.836889102503186]]], null, false),
    dam_Tishrin = /* color: #d63000 */ee.Geometry.Point([38.17848300813228, 36.380737152690656]),
    dam_Tabqa = /* color: #d63000 */ee.Geometry.Point([38.56510446209167, 35.870972690621606]),
    dam_Hurriyah = /* color: #98ff00 */ee.Geometry.Point([38.74803072140846, 35.885044983958736]),
    reservoir_Sfan = /* color: #0b4a8b */ee.Geometry.Point([42.071207224523334, 37.144020048857186]),
    dam_Ataturk = /* color: #ffc82d */ee.Geometry.Point([38.523777242267556, 37.514717078588205]),
    reservoir_Hasakah_East_dam = /* color: #00ffff */ee.Geometry.Point([40.66850348121576, 36.5944455085102]),
    reservoir_Hasakah_South_dam = /* color: #bf04c2 */ee.Geometry.Point([40.78126590402885, 36.33075273410688]),
    reservoir_Tishrin = /* color: #ff0000 */ee.Geometry.Point([38.0779886916235, 36.68440324822885]),
    cloud_shadow_water = /* color: #d63000 */ee.Geometry.Point([41.376604120839474, 35.70599468995193]),
    dam_Keban = /* color: #d63000 */ee.Geometry.Point([38.75443695803264, 38.80563795291479]),
    dam_Karakaya = /* color: #98ff00 */ee.Geometry.Point([39.13490538388588, 38.22603907121886]),
    dams_Turkey = /* color: #0b4a8b */ee.Geometry.MultiPoint(
        [[38.032466765799605, 36.86769530173293],
         [37.89200447696771, 37.05478159798348]]),
    error_urban = /* color: #d63000 */ee.Geometry.MultiPoint(
...
var end_s2 = ee.Date((year+1)+'-02-28');
var range = ee.List.sequence(1, 12, 1)


// // different for 2017 and 2021
// var year = 2017
// var start = year+'-07-01'
// var end = year+'-12-31'
// var start_s2 = ee.Date((year)+'-05-01');
// var end_s2 = ee.Date((year+1)+'-02-28');
// var range = ee.List.sequence(7, 12, 1)
...
var result_folder_name = 'error_corrected_' +'12345'
...
// // var asset_path = 'users/reachsyriagee/SY_Water/classified/' + aoi_name + '/final/'
...
var asset_path = 'users/reachsyriagee/SY_Water/classified/' + aoi_name + '/final/'+ result_folder_name + '/'
