...
var imageVisParam = {"opacity":1,"bands":["b1"],"min":-604.3828673310193,"max":787.1943472004425,"gamma":1},
    admin1 = ee.FeatureCollection("users/victormackenhauer/Admin1_2019_polygon"),
    geometry = 
    /* color: #d63000 */
    /* shown: false */
    ee.Geometry.Polygon(
        [[[48.899255965042926, 16.318376059298952],
          [49.003626082230426, 16.41851596976279],
          [49.162927840042926, 16.43432282972024],
          [49.530969832230426, 16.88164523117878],
          [49.613367293167926, 17.065530892315547],
          [49.618860457230426, 17.285954626698683],
          [49.695764754105426, 17.401309672230852],
          [49.74537673898508, 17.642932423168844],
          [49.78382888742258, 18.71281151279563],
          [49.07521072336008, 18.64516176630918],
          [48.14137283273508, 18.191738962405125],
          [47.63050857492258, 17.580104042441327],
          [47.41627517648508, 17.14494692018235],
          [47.10316482492258, 16.976901289415462],
          [46.95484939523508, 16.966393423389064],
          [46.95484939523508, 16.382294158305378],
          [46.74061599679758, 16.144993824395282],
          [47.08119216867258, 16.002476654845058],
          [47.27345291086008, 16.155546582269828],
          [47.55360427804758, 16.176650408672366],
          [47.85023513742258, 16.334857058936795],
          [48.33376915675334, 16.203512600883293],
          [48.520487797945954, 16.163129461479684],
          [48.71815505929758, 16.164779783580233]]]),
    imageVisParam2 = {"opacity":1,"bands":["smod_code"],"max":3,"gamma":1};
...
var pop = ee.ImageCollection("WorldPop/GP/100m/pop")
                  .filterMetadata('country', 'equals', 'YEM')
var pop14 = pop.filterMetadata('year', 'equals', 2014).first()
var pop15 = pop.filterMetadata('year', 'equals', 2015).first() 
var pop16 = pop.filterMetadata('year', 'equals', 2016).first()
var pop17 = pop.filterMetadata('year', 'equals', 2017).first()             
var pop18 = pop.filterMetadata('year', 'equals', 2018).first()
var pop19 = pop.filterMetadata('year', 'equals', 2019).first()                 
var pop20 = pop.filterMetadata('year', 'equals', 2020).first()

// Import VIIRS nighttime lights imagery
...
// // Refencing an urban foot print dataset
...
// // Fixing error in GHSL data
// var geometry_mask = ee.Image.constant(0).clip(geometry).mask().not()
// var build_up_area = build_up_area.first().mask(geometry_mask).unmask(0)
// var mask = build_up_area.neq(0)
// var build_up_area = build_up_area.mask(mask)

// Import provinces/countries borders
var Countries = admin1

// Apply NTL noise floors
// Everything less that 0.25 is replaced with 0
...
//Generate data for population without access
//Masks population dataset by minimum NL value
...
var pop20_noaccess = pop20.mask(pop20.gt(0).and(nl20.lt(0.05)))

//Calculate sum of people without access by province
//Sums the remaining unmasked pop count within polygons
...
var no_acc_20 = no_acc_20.select(['.*'],null,false);
Export.table.toDrive({
  collection: no_acc_20,
  description: 'no_acc_20',
  fileFormat: 'CSV',
});   
